...
#let slides(
  ...
  set page(
    width: width,
    height: height,
    margin: (x: 0.5 * space, top: 1.1 * space, bottom: 0.6 * space),
    ...
    header: context {
      ...
      if heading != none {
        ...
        set text(0.8em, weight: "bold", fill: title-color)
        v(space * 0.1)
        upper(title)
        v(space * 0.001)
        text(size: 0.8em, upper(heading.body))
      }
    },
    header-ascent: 0%,
    footer: [
  ...
  show raw.where(block: true): (item) => {
    ...
    box(
      ...
      inset: 4pt,
      radius: 8pt,
      text(fill: rgb("#a2aabc"), font: code-font, size: 10pt, item)
    )
  }
